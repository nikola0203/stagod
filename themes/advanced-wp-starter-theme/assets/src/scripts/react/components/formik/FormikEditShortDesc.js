...
function FormikEditShortDesc(props) {
  const { user_id } = props;
  const [userShortDesc, setUserShortDesc] = useState('');
  ...
  return (
    <>
      {isLoading ? (
        <Skeleton width="100%" height={130} className="rounded-4 mb-6" />
      ) : (
        <>
          <div
          ...
          >
            <div>
              <div className="h3 mb-0 me-6 fw-bold">{userShortDesc}</div>
            </div>
            <div>
              <svg
                width="32"
          ...
          >
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content p-4 p-xl-8">
                ...
                <h2 className="h3" id="modal-label-user-edit-short-desc">
                  Uredi naslov
                </h2>
                <p className="text-gray-200 fw-bold">
                  Unesite jednu rečenicu opis svojih profesionalnih veština/iskustva
                  (npr. Diplomirao engleski jezik na Kembridžu sa 10 godina
                  iskustva u podučavanju engleskog jezika i književnosti)
                </p>
                <Formik
...
export default FormikEditShortDesc;
